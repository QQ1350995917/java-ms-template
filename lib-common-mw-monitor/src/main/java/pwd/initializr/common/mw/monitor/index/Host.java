...
//        List<RPCHostProcess> process = Host.process();
  ...
  public static RPCHostMemorySwap memorySwap() {
    ...
    try {
      ...
      for (Long pid : procList) {
        ...
        try {
          ...
          rpcHostProcess.setTime(
              new RPCHostProcessTime(time.getStartTime(), time.getUser(), time.getSys(),
                  time.getTotal()));
        } catch (SigarException e) {
          e.printStackTrace();
        }
        try {
          ...
          rpcHostProcess.setMemory(
              new RPCHostProcessMemory(mem.getSize(), mem.getResident(), mem.getShare(),
                  mem.getMinorFaults(), mem.getMajorFaults(), mem.getPageFaults()));
        } catch (SigarException e) {
          e.printStackTrace();
        }
        try {
          ProcCpu cpu = SIGAR.getProcCpu(pid);
          rpcHostProcess.setCpu(
              new RPCHostProcessCpu(cpu.getPercent(), cpu.getLastTime(), cpu.getStartTime(),
                  cpu.getUser(), cpu.getSys(), cpu.getTotal()));
        } catch (SigarException e) {
          e.printStackTrace();
        }
        try {
          ProcState state = SIGAR.getProcState(pid);
          rpcHostProcess.setState(
              new RPCHostProcessState(state.getState(), state.getName(), state.getPpid(),
                  state.getTty(), state.getNice(), state.getPriority(), state.getThreads(),
                  state.getProcessor()));
        } catch (SigarException e) {
          e.printStackTrace();
        }
  ...
  public static List<RPCHostWho> who() {
