 ...
 */
public class OSHIClient {

    public static void main(String[] args) {
        Logger LOG = LoggerFactory.getLogger(OSHIClient.class);
...
//        }
    ...
    private static void printProcesses(OperatingSystem os, GlobalMemory memory) {
        ...
        // Sort by highest CPU
        List<List<OSProcess>> lists = Arrays.asList(os.getProcesses(5, ProcessSort.CPU));

        System.out.println("   PID  %CPU %MEM       VSZ       RSS Name");
        for (int i = 0; i < lists.size() && i < 5; i++) {
            List<OSProcess> osProcesses = lists.get(i);
    ...
    private static void printPowerSources(List<PowerSource> powerSources) {
        ...
        for (PowerSource pSource : powerSources) {
            sb.append(String.format("%n %s @ %.1f%%", pSource.getName(), pSource.getRemainingCapacityPercent()));
        }
