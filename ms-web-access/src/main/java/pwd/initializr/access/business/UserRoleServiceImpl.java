...
public class UserRoleServiceImpl implements UserRoleService {
  ...
  public Long insert(UserRoleBO bo) {
    UserRoleEntity entity = this.convertUserRoleBO2UserRoleEntity(bo);
  ...
  public void insert(List<UserRoleBO> bos) {
    List<UserRoleEntity> entities = bos.stream()
      .map(this::convertUserRoleBO2UserRoleEntity).collect(Collectors.toList());
    this.dao.insertByBatch(entities);
  }

  @Override
  public Long insertOrReplace(UserRoleBO bo) {
    UserRoleEntity entity = this.convertUserRoleBO2UserRoleEntity(bo);
  ...
  public PageableQueryResult<UserRoleBO> queryAllByCondition(LinkedHashSet<ScopeBO> scopes,
    ...
    List<UserRoleEntity> entities = this.dao.queryByCondition(scopes,sorts, pageIndex * pageSize, pageSize);
    if (entities == null) {
      return result;
    }
  ...
  @Override
  public Integer updateById(UserRoleBO bo){
  ...
  public UserRoleEntity convertUserRoleBO2UserRoleEntity(UserRoleBO bo){
    UserRoleEntity entity = new UserRoleEntity();
    BeanUtils.copyProperties(bo, entity);
