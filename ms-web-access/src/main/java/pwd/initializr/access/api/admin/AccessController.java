...
import pwd.initializr.access.api.admin.vo.AccessInput;
import pwd.initializr.access.api.admin.vo.AccessOutput;
import pwd.initializr.access.business.admin.AccessService;
...
@Api(
  tags = "Access信息结构",
  value = "AccessManageApi",
  description = "[列表查询，详情查询，启/禁用，删除，新增，修改]"
)
@RestController(value = "Access")
@RequestMapping(value = "/api/admin/access")
@Slf4j
public class AccessController extends pwd.initializr.common.web.api.admin.AdminController implements AccessApi {
  ...
  public void list(String scopes, String sorts, String page) {
    ...
    PageableQueryResult<AccessBO> pageableQueryResult = service.queryAllByCondition(scopeBOS,
      sortBOS, pageInput.getIndex(), pageInput.getSize());
    PageOutput<AccessOutput> result = new PageOutput<>();
    pageableQueryResult.getElements().forEach(bo -> {
      AccessOutput output = new AccessOutput();
  ...
  public void detail(@Valid @NotNull(message = "参数不能为空") Long id) {
    AccessBO bo = service.queryById(id);
    AccessOutput output = new AccessOutput();
    BeanUtils.copyProperties(bo,output);
    outputData(output);
  }
  ...
  @Override
  public void create(@Valid @NotNull(message = "参数不能为空") AccessInput input) {
  ...
  @Override
  public void create(@Valid @NotNull(message = "参数不能为空") List<AccessInput> input) {
  List<AccessBO> bos = input.stream().map(this::convertAccessInput2AccessBO)
  ...
  @Override
  public void createOrReplace(@Valid @NotNull(message = "参数不能为空") AccessInput input) {
    AccessBO bo = new AccessBO();
    BeanUtils.copyProperties(input,bo);
  ...
  @Override
  public void createOrReplace(@Valid @NotNull(message = "参数不能为空") List<AccessInput> input) {
  List<AccessBO> bos = input.stream().map(this::convertAccessInput2AccessBO)
    .collect(Collectors.toList());
  ...
  public void update(@Valid @NotNull(message = "参数不能为空") Long id,
    @Valid @NotNull(message = "参数不能为空") AccessInput input) {
    AccessBO bo = new AccessBO();
    BeanUtils.copyProperties(input,bo);
    bo.setId(id);
  ...
  private AccessBO convertAccessInput2AccessBO(AccessInput input){
    AccessBO bo = new AccessBO();
    BeanUtils.copyProperties(input,bo);
    return bo;
  }
