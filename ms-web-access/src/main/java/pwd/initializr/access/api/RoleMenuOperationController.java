...
import pwd.initializr.access.api.vo.RoleMenuOperationInput;
import pwd.initializr.access.api.vo.RoleMenuOperationOutput;
import pwd.initializr.access.business.RoleMenuOperationService;
...
*
* <h1>RoleMenuOperation控制层接口实现</h1>
*
...
*/
@Api(
  tags = "RoleMenuOperation信息结构",
  value = "RoleMenuOperationManageApi",
  description = "[列表查询，详情查询，启/禁用，删除，新增，修改]"
)
@RestController(value = "RoleMenuOperation")
@RequestMapping(value = "/api/role/menu/operation")
@Slf4j
public class RoleMenuOperationController extends pwd.initializr.common.web.api.admin.AdminController implements RoleMenuOperationApi {
  ...
  public void list(String scopes, String sorts, String page) {
    ...
    PageableQueryResult<RoleMenuOperationBO> pageableQueryResult = service
      .queryAllByCondition(scopeBOS, sortBOS, pageInput.getIndex(), pageInput.getSize());
    PageOutput<RoleMenuOperationOutput> result = new PageOutput<>();
    pageableQueryResult.getElements().forEach(bo -> {
      RoleMenuOperationOutput output = new RoleMenuOperationOutput();
  ...
  public void detail(@Valid @NotNull(message = "参数不能为空") Long id) {
    RoleMenuOperationBO bo = service.queryById(id);
    RoleMenuOperationOutput output = new RoleMenuOperationOutput();
    BeanUtils.copyProperties(bo,output);
    outputData(output);
  }
  ...
  @Override
  public void create(@Valid @NotNull(message = "参数不能为空") RoleMenuOperationInput input) {
  ...
  @Override
  public void create(@Valid @NotNull(message = "参数不能为空") List<RoleMenuOperationInput> input) {
    List<RoleMenuOperationBO> bos = input.stream().map(this::convertRoleMenuOperationInput2RoleMenuOperationBO)
  ...
  @Override
  public void createOrReplace(@Valid @NotNull(message = "参数不能为空") RoleMenuOperationInput input) {
    RoleMenuOperationBO bo = new RoleMenuOperationBO();
    BeanUtils.copyProperties(input,bo);
  ...
  @Override
  public void createOrReplace(@Valid @NotNull(message = "参数不能为空") List<RoleMenuOperationInput> input) {
    List<RoleMenuOperationBO> bos = input.stream().map(this::convertRoleMenuOperationInput2RoleMenuOperationBO)
      .collect(Collectors.toList());
  ...
  @Valid @NotNull(message = "参数不能为空") RoleMenuOperationInput input) {
    RoleMenuOperationBO bo = new RoleMenuOperationBO();
    BeanUtils.copyProperties(input,bo);
  ...
  private RoleMenuOperationBO convertRoleMenuOperationInput2RoleMenuOperationBO(RoleMenuOperationInput input){
    RoleMenuOperationBO bo = new RoleMenuOperationBO();
    BeanUtils.copyProperties(input,bo);
    return bo;
  }
