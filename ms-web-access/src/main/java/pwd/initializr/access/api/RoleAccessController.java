...
import pwd.initializr.access.api.vo.RoleAccessInput;
import pwd.initializr.access.api.vo.RoleAccessOutput;
import pwd.initializr.access.business.RoleAccessService;
import pwd.initializr.access.business.bo.RoleAccessBO;
...
@Api(
  tags = "RoleAccess信息结构",
  value = "RoleAccessManageApi",
  description = "[列表查询，详情查询，启/禁用，删除，新增，修改]"
)
@RestController(value = "RoleAccess")
@RequestMapping(value = "/api/role/access")
@Slf4j
public class RoleAccessController extends pwd.initializr.common.web.api.admin.AdminController implements RoleAccessApi {
  ...
  public void list(String scopes, String sorts, String page) {
    ...
    PageableQueryResult<RoleAccessBO> pageableQueryResult = service.queryAllByCondition(scopeBOS,
      sortBOS, pageInput.getIndex(), pageInput.getSize());
    PageOutput<RoleAccessOutput> result = new PageOutput<>();
    pageableQueryResult.getElements().forEach(bo -> {
      RoleAccessOutput output = new RoleAccessOutput();
  ...
  public void detail(@Valid @NotNull(message = "参数不能为空") Long id) {
    RoleAccessBO bo = service.queryById(id);
    RoleAccessOutput output = new RoleAccessOutput();
    BeanUtils.copyProperties(bo,output);
    outputData(output);
  }
  ...
  @Override
  public void create(@Valid @NotNull(message = "参数不能为空") RoleAccessInput input) {
  ...
  @Override
  public void create(@Valid @NotNull(message = "参数不能为空") List<RoleAccessInput> input) {
  List<RoleAccessBO> bos = input.stream().map(this::convertRoleAccessInput2RoleAccessBO)
  ...
  @Override
  public void createOrReplace(@Valid @NotNull(message = "参数不能为空") RoleAccessInput input) {
    RoleAccessBO bo = new RoleAccessBO();
    BeanUtils.copyProperties(input,bo);
  ...
  @Override
  public void createOrReplace(@Valid @NotNull(message = "参数不能为空") List<RoleAccessInput> input) {
  List<RoleAccessBO> bos = input.stream().map(this::convertRoleAccessInput2RoleAccessBO)
    .collect(Collectors.toList());
  ...
  public void update(@Valid @NotNull(message = "参数不能为空") Long id,
    @Valid @NotNull(message = "参数不能为空") RoleAccessInput input) {
    RoleAccessBO bo = new RoleAccessBO();
    BeanUtils.copyProperties(input,bo);
    bo.setAid(id);
  ...
  private RoleAccessBO convertRoleAccessInput2RoleAccessBO(RoleAccessInput input){
    RoleAccessBO bo = new RoleAccessBO();
    BeanUtils.copyProperties(input,bo);
    return bo;
  }
