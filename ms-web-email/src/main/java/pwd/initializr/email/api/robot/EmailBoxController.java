...
import pwd.initializr.email.api.robot.vo.EmailBoxInput;
import pwd.initializr.email.api.robot.vo.EmailBoxOutput;
import pwd.initializr.email.business.EmailBoxService;
import pwd.initializr.email.business.bo.EmailBoxBO;

/**
 * project-generator-test@ms-web-initializr
 *
 * <h1>EmailBox控制层接口实现</h1>
 *
...
@Api(
    tags = "EmailBox信息结构",
    value = "EmailBoxManageApi",
    description = "[列表查询，详情查询，启/禁用，删除，新增，修改]"
)
@RestController(value = "EmailBox")
@RequestMapping(value = "/api/robot/email/Box")
@Slf4j
public class EmailBoxController extends
    pwd.initializr.common.web.api.admin.AdminController implements EmailBoxApi {
    ...
    public void list(String scopes, String sorts, String page) {
        ...
        PageableQueryResult<EmailBoxBO> pageableQueryResult = service
            .queryAllByCondition(scopeBOS, sortBOS, pageInput.getIndex(), pageInput.getSize());
        PageOutput<EmailBoxOutput> result = new PageOutput<>();
        pageableQueryResult.getElements().forEach(bo -> {
            EmailBoxOutput output = new EmailBoxOutput();
    ...
    public void detail(@Valid @NotNull(message = "参数不能为空") Long id) {
        EmailBoxBO bo = service.queryById(id);
        EmailBoxOutput output = new EmailBoxOutput();
        BeanUtils.copyProperties(bo, output);
        outputData(output);
    }
    ...
    @Override
    public void create(@Valid @NotNull(message = "参数不能为空") EmailBoxInput input) {
    ...
    @Override
    public void create(@Valid @NotNull(message = "参数不能为空") List<EmailBoxInput> input) {
        Set<EmailBoxBO> bos = new HashSet<>();
        for (EmailBoxInput item : input) {
            EmailBoxBO bo = new EmailBoxBO();
    ...
    @Override
    public void createOrReplace(@Valid @NotNull(message = "参数不能为空") EmailBoxInput input) {
        EmailBoxBO bo = new EmailBoxBO();
        BeanUtils.copyProperties(input, bo);
    ...
    @Override
    public void createOrReplace(@Valid @NotNull(message = "参数不能为空") List<EmailBoxInput> input) {
        LinkedList<EmailBoxBO> bos = new LinkedList<>();
        for (EmailBoxInput item : input) {
            EmailBoxBO bo = new EmailBoxBO();
            BeanUtils.copyProperties(item, bo);
            bos.add(bo);
    ...
    public void update(@Valid @NotNull(message = "参数不能为空") Long id,
        @Valid @NotNull(message = "参数不能为空") EmailBoxInput input) {
        EmailBoxBO bo = new EmailBoxBO();
        BeanUtils.copyProperties(input, bo);
