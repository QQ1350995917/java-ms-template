...
import pwd.initializr.edu.api.admin.vo.EduTermInput;
import pwd.initializr.edu.api.admin.vo.EduTermOutput;
...
@Api(
  tags = "EduTerm信息结构",
  value = "EduTermManageApi",
  description = "[列表查询，详情查询，启/禁用，删除，新增，修改]"
)
@RestController(value = "EduTerm")
@RequestMapping(value = "/api/edu/term")
@Slf4j
public class EduTermController extends pwd.initializr.common.web.api.admin.AdminController implements EduTermApi {

  @Autowired
  private EduTermService service;
  ...
  public void list(String scopes, String sorts, String page) {
    ...
    PageableQueryResult<EduTermBO> pageableQueryResult = service.queryAllByCondition(scopeBOS,
      sortBOS, pageInput.getIndex(), pageInput.getSize());
    PageOutput<EduTermOutput> result = new PageOutput<>();
    pageableQueryResult.getElements().forEach(bo -> {
      EduTermOutput output = new EduTermOutput();
  ...
  public void detail(@Valid @NotNull(message = "参数不能为空") Long id) {
    EduTermBO bo = service.queryById(id);
    EduTermOutput output = new EduTermOutput();
    BeanUtils.copyProperties(bo,output);
    outputData(output);
  }
  ...
  @Override
  public void create(@Valid @NotNull(message = "参数不能为空") EduTermInput input) {
  ...
  @Override
  public void create(@Valid @NotNull(message = "参数不能为空") List<EduTermInput> input) {
  List<EduTermBO> bos = input.stream().map(this::convertEduTermInput2EduTermBO)
  ...
  @Override
  public void createOrReplace(@Valid @NotNull(message = "参数不能为空") EduTermInput input) {
    EduTermBO bo = new EduTermBO();
    BeanUtils.copyProperties(input,bo);
  ...
  @Override
  public void createOrReplace(@Valid @NotNull(message = "参数不能为空") List<EduTermInput> input) {
  List<EduTermBO> bos = input.stream().map(this::convertEduTermInput2EduTermBO)
    .collect(Collectors.toList());
  ...
  public void update(@Valid @NotNull(message = "参数不能为空") Long id,
    @Valid @NotNull(message = "参数不能为空") EduTermInput input) {
    EduTermBO bo = new EduTermBO();
    BeanUtils.copyProperties(input,bo);
    bo.setId(id);
  ...
  private EduTermBO convertEduTermInput2EduTermBO(EduTermInput input){
    EduTermBO bo = new EduTermBO();
    BeanUtils.copyProperties(input,bo);
    return bo;
  }
