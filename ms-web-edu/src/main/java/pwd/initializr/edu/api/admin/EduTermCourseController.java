...
import pwd.initializr.edu.api.admin.vo.EduTermCourseInput;
import pwd.initializr.edu.api.admin.vo.EduTermCourseOutput;
 ...
 */
@Api(
  tags = "EduTermCourse信息结构",
  value = "EduTermCourseManageApi",
  description = "[列表查询，详情查询，启/禁用，删除，新增，修改]"
)
@RestController(value = "EduTermCourse")
@RequestMapping(value = "/api/edu/term/course")
@Slf4j
public class EduTermCourseController extends pwd.initializr.common.web.api.admin.AdminController implements EduTermCourseApi {

  @Autowired
  private EduTermCourseService service;
  ...
  public void list(String scopes, String sorts, String page) {
    ...
    PageableQueryResult<EduTermCourseBO> pageableQueryResult = service.queryAllByCondition(scopeBOS,
      sortBOS, pageInput.getIndex(), pageInput.getSize());
    PageOutput<EduTermCourseOutput> result = new PageOutput<>();
    pageableQueryResult.getElements().forEach(bo -> {
      EduTermCourseOutput output = new EduTermCourseOutput();
  ...
  public void detail(@Valid @NotNull(message = "参数不能为空") Long id) {
    EduTermCourseBO bo = service.queryById(id);
    EduTermCourseOutput output = new EduTermCourseOutput();
    BeanUtils.copyProperties(bo,output);
    outputData(output);
  }
  ...
  @Override
  public void create(@Valid @NotNull(message = "参数不能为空") EduTermCourseInput input) {
  ...
  @Override
  public void create(@Valid @NotNull(message = "参数不能为空") List<EduTermCourseInput> input) {
  List<EduTermCourseBO> bos = input.stream().map(this::convertEduTermCourseInput2EduTermCourseBO)
  ...
  @Override
  public void createOrReplace(@Valid @NotNull(message = "参数不能为空") EduTermCourseInput input) {
    EduTermCourseBO bo = new EduTermCourseBO();
    BeanUtils.copyProperties(input,bo);
  ...
  @Override
  public void createOrReplace(@Valid @NotNull(message = "参数不能为空") List<EduTermCourseInput> input) {
  List<EduTermCourseBO> bos = input.stream().map(this::convertEduTermCourseInput2EduTermCourseBO)
    .collect(Collectors.toList());
  ...
  public void update(@Valid @NotNull(message = "参数不能为空") Long id,
    @Valid @NotNull(message = "参数不能为空") EduTermCourseInput input) {
    EduTermCourseBO bo = new EduTermCourseBO();
    BeanUtils.copyProperties(input,bo);
    bo.setId(id);
  ...
  private EduTermCourseBO convertEduTermCourseInput2EduTermCourseBO(EduTermCourseInput input){
    EduTermCourseBO bo = new EduTermCourseBO();
    BeanUtils.copyProperties(input,bo);
    return bo;
  }
